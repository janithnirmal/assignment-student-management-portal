...
$userCheck_rs = Database::search("SELECT * FROM academic_officer WHERE `nic` = '" . $nic . "' ");
...
if ($userCheck_num == 0) {
    ...
    $userCheck2_rs = Database::search("SELECT * FROM academic_officer WHERE nic = '" . $nic . "' AND verification_code = '" . $vc . "' ");
    ...
    if ($userCheck2_num == 0) {
        ...
        if (empty($npass) || empty($rtpass)) {
            ...
            $newVc = uniqid();
            Database::iud("UPDATE academic_officer SET verification_code = '" . $newVc . "', password='" . $npass . "', verification_status='1' WHERE nic = '" . $nic . "' ");
